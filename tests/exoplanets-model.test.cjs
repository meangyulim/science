...
test('radial velocity: mass, inclination, period and recession sign',()=>{
  ...
  close(M.rv(0,{...p,inclination:0}).value,0);
  close(M.rv(0,{...p,inclination:30}).k,M.rv(0,p).k/2);
...
test('transit: exact circle overlap, radius squared, front/back and missing transit',()=>{
  ...
  close(M.transit(p.period/4,{...p,inclination:75}).value,100);
  assert.equal(M.transit(0,{...p,inclination:0}).canTransit,false);
...
test('direct imaging: projected angular separation, glare, distance and brightness',()=>{
  ...
  close(M.direct(P/4,{...p,inclination:90}).separation,0);
  close(M.direct(P/4,{...p,inclination:0}).separation,.4);
